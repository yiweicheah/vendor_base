-- Per-event P&L using revenue − COGS:
--   gross_profit = SUM((unit_price − cost) × qty) across sold lines
--     where cost = avg_cost_myr for cards, sealed_reference_price for sealed.
-- Matches what TransactionCard shows per-transaction, so a sum across an
-- event's transactions equals the event's gross_profit.
-- profit_complete = false if any sold line lacks cost data (UI shows '~').
-- Walk-in (NULL event_id) appears as a single row sorted last.

-- Return type changed (removed opening_stock, closing_stock; restored
-- card_sold_total, profit_complete), so the old function must be dropped first.
DROP FUNCTION IF EXISTS public.get_org_event_breakdown(uuid);
...
agg AS (
  SELECT
    ...
    -- Sold qty (cards + sealed) — denominator for profit_complete.
    COALESCE(SUM(CASE WHEN type IN ('card','sealed') AND side='out' THEN qty END), 0)::integer
      AS sold_total,
    -- Sold qty where cost basis is known:
    --   card: avg_cost_myr present, OR card_external_id NULL (untracked = treat as 0 cost)
    --   sealed: sealed_reference_price present
    COALESCE(SUM(CASE
      WHEN type='card'   AND side='out'
           AND (card_external_id IS NULL OR avg_cost_myr IS NOT NULL)
        THEN qty
      WHEN type='sealed' AND side='out' AND sealed_reference_price IS NOT NULL
        THEN qty
    END), 0)::integer AS sold_with_cost,
    -- Per-event gross profit:
    --   card sales:    (unit_price - avg_cost_myr) * qty  (avg_cost = 0 for untracked bulk)
    --   sealed sales:  (unit_price - sealed_reference_price) * qty
    COALESCE(SUM(CASE
      WHEN type='card'   AND side='out'
           AND (card_external_id IS NULL OR avg_cost_myr IS NOT NULL)
        THEN (unit_price - COALESCE(avg_cost_myr, 0)) * qty
      WHEN type='sealed' AND side='out' AND sealed_reference_price IS NOT NULL
        THEN (unit_price - sealed_reference_price) * qty
...
SELECT
  ...
  COALESCE(a.sold_total, 0)                           AS card_sold_total,
  (COALESCE(a.sold_total, 0) = 0
    OR COALESCE(a.sold_total, 0) = COALESCE(a.sold_with_cost, 0))
                                                      AS profit_complete,
